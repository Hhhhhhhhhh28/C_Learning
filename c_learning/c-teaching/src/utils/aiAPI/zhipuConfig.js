...
export const TOOL_MESSAGE = {
  ...
  topic: (chapter, topics, questionCount) => {
    return `你是一个严谨的前端语言课程出题专家，我会给你如下信息：
            '''
            {chapter: '${chapter}',
  ...
  proficiency: (question, userAnswer) => {
    return `你是一个严谨的根据前端语言知识点题目给出用户对该知识点熟练度专家，我会给你如下信息：
            '''
            {question: ${question},
...
export const generateConfigTalk = question => {
  return JSON.stringify({
    ...
    messages: [
      {
        role: 'system',
        content: '简单回答，100字以内，给一点代码示例，前端语言有关',
      },
      {
        role: 'user',
        content: question,
